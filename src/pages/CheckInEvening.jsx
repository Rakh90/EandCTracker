  ...
  return (
    <div>
      ...
      <div className="card">
        ...
        <Slider label="Intensity" value={log.movement_intensity} onChange={(v) => patch({ movement_intensity: v })} min={1} max={10} />
      </div>
      ...
      <div className="card">
        ...
        {SYMPTOMS.filter((s) => (log[s.key] ?? 0) > 0).map((s) => (
          <Slider key={s.key} label={`${s.label} severity`} value={log[s.key]} onChange={(v) => patch({ [s.key]: v })} min={1} max={10} />
        ))}
        <div className="field">
      ...
      <div className="card">
        ...
        <Slider label="Task completion" value={log.exec_tasks} onChange={(v) => patch({ exec_tasks: v })} min={1} max={10} />
        <Slider label="Problem solving" value={log.exec_problems} onChange={(v) => patch({ exec_problems: v })} min={1} max={10} />
        <Slider label="Organization" value={log.exec_organization} onChange={(v) => patch({ exec_organization: v })} min={1} max={10} />
      </div>

      <div className="card">
        <h3>End-of-day state</h3>
        <Slider label="Physical energy" value={log.pm_phys_energy} onChange={(v) => patch({ pm_phys_energy: v })} min={1} max={10} />
        <Slider label="Mental energy" value={log.pm_mental_energy} onChange={(v) => patch({ pm_mental_energy: v })} min={1} max={10} />
        <Slider label="Mood" value={log.pm_mood} onChange={(v) => patch({ pm_mood: v })} min={1} max={10} />
        <Slider label="Stress (day average)" value={log.stress_avg} onChange={(v) => patch({ stress_avg: v })} min={1} max={10} />
